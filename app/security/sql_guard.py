...
def add_nolock_hints(sql: str) -> str:
    ...
    mistaken for aliases and are never consumed by the match. Skips tables
    that already have a NOLOCK hint to avoid duplication."""
    ...
    def replacer(match):
        ...
        table = match.group(2)
        alias = match.group(4)
        already_has_nolock = match.group(5)

        if already_has_nolock:
            return match.group(0)  # leave as-is, don't duplicate
